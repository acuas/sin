...
RUN mv go /usr/local
RUN echo 'export GOROOT=/usr/local/go' >> /root/.bashrc && echo 'export GOPATH=$HOME/go' >> /root/.bashrc && echo 'export PATH=$GOPATH/bin:$GOROOT/bin:$PATH' >> /root/.bashrc
WORKDIR /go/src/github.com/sin
COPY . . 
RUN go mod download
RUN go build -o main sin.go
